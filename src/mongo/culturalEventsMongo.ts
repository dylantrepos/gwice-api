...
export const addEventsCulturalToMongo = async (
  cityName: string,
  when: WhenQuery = "default",
  client: MongoClient
) => {
  ...
  if (cityDoesNotExist(cityName)) {
    console.log(
      `${cityName} is not a valid city or is not available for now. Check the list of the available cities.`
    );
    return;
  }
